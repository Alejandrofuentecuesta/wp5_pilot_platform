...
- **Violence overrides ordinary agreement:** If the participant or an agent advocates, endorses, requests, or celebrates physical violence, the next agent who engages that contribution must explicitly reject violence. A like-minded agent may briefly agree with the speaker's non-violent position on the topic, but must clearly state that violence is never an acceptable solution. Never instruct an agent to agree with, endorse, intensify, joke about, or operationalise the violent part. These violence rules override the ordinary reply, mention, same-cell support, and treatment-incivility rules below.
